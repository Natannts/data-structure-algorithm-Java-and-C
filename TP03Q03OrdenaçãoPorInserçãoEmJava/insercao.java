...
class Lista{
    ...
    private void insertionSort(){
        for(int i=1; i<contador; i++){
            Series tmp = serie[i].clone(); numMovimentacoes++;
            int j = i-1;

            while((j>=0) && serie[j].getIdioma().compareTo(tmp.getIdioma()) > 0 
                || (j>=0) && serie[j].getIdioma().compareTo(tmp.getIdioma()) == 0 
...
class insercao{
    ...
    public static void main(String []args) throws Exception{
       ...
       String arqLog = "625188\t" + Integer.toString(serie.numComparacoes) 
        ...
        Arq.openWriteClose("matrícula_insercao.txt","UTF-8", arqLog);
