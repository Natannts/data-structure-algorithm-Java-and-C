    ...
    public static void main(String []args) throws Exception{
		...
		do{
			...
			if(!compare(entrada, "FIM")) {
                ...
                int dir = quantEntrada, esq = 0, meio;
                while(esq <= dir && !encontrado){
                    meio = (esq + dir)/2;
                    Series j = NomeSeries[meio];
                    if(entrada.compareTo(j.getNome()) == 0){
                        encontrado = true;
                        esq = quantEntrada;
                        comps ++;
                    }else if(entrada.compareTo(j.getNome()) > 0){
                        esq = meio + 1;
                        comps += 2;
                    }else {
                        dir = meio - 1;
		...
		Arq.openWrite("625188_binaria.txt");
