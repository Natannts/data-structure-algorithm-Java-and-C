...
void selecaoRec(Series *series, int n, int i)
{
    ...
    if (i < n)
    {
        ...
        for ( j = 0; j < (n-1); j++)
        {
            ...
            if (strcmp(series[j].paisOrigem, series[j + 1].paisOrigem) > 0) {
                swap(series, j, j + 1);
                numComparacoes++;
            }
            else if (strcmp(series[j].paisOrigem, series[j + 1].paisOrigem) == 0){
                if ((strcmp(series[j].nome, series[j + 1].nome) > 0)){
                    swap(series, j, j + 1);
                }
                numComparacoes +=2;
            }
        ...
        return selecaoRec(series, n, i + 1);
    }
}
