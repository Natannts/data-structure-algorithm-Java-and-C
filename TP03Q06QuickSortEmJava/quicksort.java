...
class Lista{
    ...
    public void quicksort(int esq, int dir) {
        int i=esq, j=dir;
        String pivo = "", pivoNome = "";
        pivo = serie[(dir+esq)/2].getPaisOrigem();
        pivoNome = serie[(dir+esq)/2].getNome();

        while(i<=j){
            while(serie[i].getPaisOrigem().compareTo(pivo)<0 
                || serie[i].getPaisOrigem().compareTo(pivo) == 0 
                    && serie[i].getNome().compareTo(pivoNome)<0) i++;

            while(serie[j].getPaisOrigem().compareTo(pivo)>0
                ||serie[j].getPaisOrigem().compareTo(pivo) == 0 
                    && serie[j].getNome().compareTo(pivoNome)>0) j--;
...
class quicksort{
    ...
    public static void main(String []args) throws Exception{
       ...
       String arqLog = "625188\t" + Integer.toString(serie.numComparacoes) 
        ...
        Arq.openWriteClose("matrícula_quicksort.txt","UTF-8", arqLog);
