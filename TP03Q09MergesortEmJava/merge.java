...
class Lista{
     ...
     */
    private void mergesort(Lista aux, int n){
        if(n >= 2){
            int meio = n / 2;
            Lista esq = new Lista(meio);
            Lista dir = new Lista(n - meio);

            //Copiar a lista de series em duas sub-listas:
            for(int i = 0; i < meio; i++) esq.serie[i] = aux.serie[i].clone();
            for(int i = meio; i < n; i++) dir.serie[i - meio] = aux.serie[i].clone();

            //Executar recursividade:
            mergesort(esq, meio);
            mergesort(dir, n - meio);

            //Juntar ("merge") os subarrays:
            merge(aux, esq, dir, meio, n-meio);
        }
    }
  /**
     * Realiza a ordenacao propiamente dita
     * @param aux lista auxiliar que armazenara a lista ordenada completa
     * @param esq sub-lista (esquerda) da lista auxiliar
     * @param dir sub-lista (direita) da lista auxiliar
     * @param indexEsq indice do ultimo elemento para a sub-lista da esquerda
     * @param indexDir indice do ultimo elemento para a sub-lista da direita
     */
    private void merge(Lista aux, Lista esq, Lista dir, int indexEsq, int indexDir){
        int i = 0, j = 0, k = 0;

        //Realizar a ordenacao propiamente dita:
        while((i < indexEsq) && (j < indexDir)){

            if(esq.serie[i].getQntEpisodios() < dir.serie[j].getQntEpisodios()
                || esq.serie[i].getQntEpisodios() == dir.serie[j].getQntEpisodios()
                    && esq.serie[i].getNome() .compareTo(dir.serie[j].getNome()) < 0){ 
                aux.serie[k++] = esq.serie[i++].clone();
            }else aux.serie[k++] = dir.serie[j++].clone();

            numComparacoes++; numMovimentacoes++;
        }

        //Copiar o restante dos elementos que nao tiverem sido copiados:
        while(i < indexEsq) aux.serie[k++] = esq.serie[i++].clone();
        while(j < indexDir) aux.serie[k++] = dir.serie[j++].clone();
    }
